...
mongoose.connect("mongodb://ilhan:12345678@127.0.0.1:27017/dku?authSource=admin")
...
app.post('/create-assignment', (req, res) => {
  ...
  const ass = { Id: uuidv4(), Name: req.body.Name, Hours: hours, State: "Pending", Info: { Description: des, Date: date, Participants: [stu] }, Student: stu }
...
app.get('/get-colleagues', (req, res) => {
  if (!req.session.loggedin || req.session.Type != "Student") {res.redirect("/login");return;}
  Students.find({ Id: req.session.Id }, (err, prof) => {
    if(err) console.log(err)
    Students.find({ Professor: prof[0].Professor }, {Id:1, Name:1}, (err, results) => {
      if(err) console.log(err)
      //console.log(results)
      res.send({Colleagues: results})
    })
  })
})
...
app.get('/get-students', (req, res) => {
  ...
  Students.aggregate([{ $lookup: {
    ...
    , {$sort:{"Class":-1, "Name":1, "Assignments.Info.Date":1}} ], (err, results) => {
    ...
    if (resultsa.length > 0) {
      ...
      Students.find({ Id: resultsa[0].Student }, (err, results) => {
        if (results[0].Professor == req.session.Id) {
          ...
          for(participant of resultsa[0].Info.Participants){
             console.log("id: ", participant)    
              Students.updateOne({ Id: participant }, { $inc: { "Hours": resultsa[0].Hours } }, (err) => {
                if (err) console.log(err)
              })
          }
